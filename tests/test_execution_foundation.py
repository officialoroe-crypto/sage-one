...
def test_resource_guard_thresholds():
    ...
    low = ResourceSnapshot(20, 4, None, time.time())
    soft = ResourceSnapshot(55, 4, None, time.time())
    hard = ResourceSnapshot(75, 4, None, time.time())

    assert guard.allowed(low)
    assert not guard.should_throttle(low)
    assert guard.should_throttle(soft)
    assert not guard.should_stop(soft)
    assert not guard.allowed(hard)
    assert guard.should_stop(hard)
...
def test_scheduler_blocks_at_hard_cpu_limit():
    ...
    scheduler = ExecutionScheduler(max_workers=1, guard=guard)
    try:
        hard = ResourceSnapshot(90, 4, None, time.time())
