...
class ResourceGuard:
    ...
    SAGE should keep the user's laptop responsive. Local work is allowed below
    the hard ceiling, throttled in the busy/heavy bands, and blocked at the
    critical level. Heavy AI work should be routed to cloud/background work
    instead of silently falling back to local Ollama.
    """
    ...
    def snapshot(self) -> ResourceSnapshot:
        if psutil is not None:
            ...
            # Dependency-free fallback: process CPU only. This is deliberately
            # conservative for scheduling decisions when host metrics are unavailable.
            start = os.times()
