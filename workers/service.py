...
class BackgroundWorker:
    """Dispatch persisted pending tasks through the bounded local scheduler."""
    ...
    def dispatch_once(self) -> dict[str, Any]:
        ...
        capacity = max(0, min(self.config.max_dispatch, self.scheduler.max_workers) - len(self._in_flight))
        if capacity == 0:
            return {
                "success": True,
                "status": "at_capacity",
        ...
        for task in pending:
            ...
            with self._lock:
                if task_id in self._in_flight:
                    continue
            try:
                scheduler_id = self.scheduler.submit(self.task_executor, task_id, task_id=task_id)
            except RuntimeError:
                break
            with self._lock:
    ...
    def start(self) -> bool:
        with self._lock:
            if self._thread and self._thread.is_alive():
                return False
            self._stop_event.clear()
            self._thread = Thread(target=self.run_forever, name="sage-background-worker", daemon=True)
            self._thread.start()
            return True
