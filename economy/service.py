...
def evolution_progress(achievement: int) -> dict:
    ...
    return {"current_threshold": current_threshold, "next_threshold": next_threshold, "next_tier": next_tier, "ratio": max(0.0, min(1.0, ratio))}
...
def grant_sparks(db: Session, owner_key: str, amount: int, reason: str, reference: str | None = None, metadata: dict | None = None) -> SparkLedgerEntry:
    if amount <= 0:
    ...
    entry = SparkLedgerEntry(owner_key=owner_key, delta=amount, balance_after=wallet.balance, reason=reason, reference=reference, metadata_json=json.dumps(metadata or {}, sort_keys=True))
    db.add(entry)
    db.commit()
    db.refresh(entry)
    return entry
...
def spend_sparks(db: Session, owner_key: str, amount: int, reason: str, reference: str | None = None, metadata: dict | None = None) -> SparkLedgerEntry:
    if amount <= 0:
...
def record_achievement(db: Session, owner_key: str, amount: int, reason: str, *, commit: bool = True) -> EvolutionProfile:
    if amount <= 0:
...
def snapshot(db: Session, owner_key: str) -> dict:
    ...
    entries = db.scalars(select(SparkLedgerEntry).where(SparkLedgerEntry.owner_key == owner_key).order_by(SparkLedgerEntry.created_at.desc()).limit(50)).all()
    return {"spark": {"name": "SAGE Spark", "balance": wallet.balance, "lifetime_earned": wallet.lifetime_earned, "lifetime_spent": wallet.lifetime_spent}, "evolution": {"lifetime_achievement": evolution.lifetime_achievement, "tier": evolution.tier, "stage": evolution.stage, "progress": evolution_progress(evolution.lifetime_achievement)}, "ledger": [{"id": entry.id, "delta": entry.delta, "balance_after": entry.balance_after, "reason": entry.reason, "reference": entry.reference, "created_at": entry.created_at.isoformat()} for entry in entries]}
...
def evolution_simulation(db: Session, owner_key: str, target_achievement: int, duration_ms: int = 3000) -> dict:
    ...
    milestones = [{"achievement": threshold, "tier": tier} for threshold, tier in EVOLUTION_TIERS if low < threshold <= high]
    if target_achievement < start:
        milestones = list(reversed(milestones))
    return {"simulation": True, "mutated": False, "duration_ms": duration_ms, "direction": "up" if target_achievement >= start else "down", "from": {"lifetime_achievement": start, "tier": start_tier, "stage": start_stage, "progress": evolution_progress(start)}, "to": {"lifetime_achievement": target_achievement, "tier": target_tier, "stage": target_stage, "progress": evolution_progress(target_achievement)}, "milestones": milestones}
