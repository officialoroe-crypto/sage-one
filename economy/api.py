...
def verified_evolution_achievement(request: VerifiedAchievementRequest, claims: dict = Depends(authenticate_request)):
    try:
        with SessionLocal() as db:
            event, created = record_verified_achievement(db, _owner(claims), request.amount, request.reason, request.source_type, request.source_id, request.evidence)
            profile = get_evolution(db, _owner(claims))
            return {"success": True, "created": created, "event": {"id": event.id, "source_type": event.source_type, "source_id": event.source_id, "amount": event.amount, "verification_status": event.verification_status, "created_at": event.created_at.isoformat()}, "evolution": {"lifetime_achievement": profile.lifetime_achievement, "tier": profile.tier, "stage": profile.stage}}
    except ValueError as exc:
...
def evolution_me(claims: dict = Depends(authenticate_request)):
    with SessionLocal() as db:
        profile = get_evolution(db, _owner(claims))
        return {"success": True, "lifetime_achievement": profile.lifetime_achievement, "tier": profile.tier, "stage": profile.stage}
...
def owner_spark_set(request: OwnerSparkRequest, claims: dict = Depends(authenticate_request)):
    ...
    with SessionLocal() as db:
        ...
        return {"success": True, "spark": {"balance": wallet.balance, "lifetime_earned": wallet.lifetime_earned, "lifetime_spent": wallet.lifetime_spent}}
