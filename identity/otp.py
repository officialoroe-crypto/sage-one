...
This checkpoint creates the verification state machine without pretending to
send SMS. A real provider adapter can be plugged into `OTPProvider.send` and
`OTPProvider.verify` later; codes are never persisted in the user profile.
"""
...
class OTPManager:
    ...
    def create_challenge(self, phone: str) -> tuple[OTPChallenge, str]:
        phone = phone.strip()
        if not phone:
