...
class SageWorker:
    ...
    def run_goal(self, goal, session_id=None, priority=3):
        try:
            result = orchestrator.execute_goal(goal=goal, session_id=session_id, priority=priority)
    ...
    def _local_execution_allowed(self):
        ...
        allowed = local_execution_allowed(task_class, snapshot.cpu_percent)
        if allowed:
            return True, None
    ...
    def claim(self):
        ...
        return tasks.claim_next(worker_id=self.worker_id, lease_seconds=self.lease_seconds)

    def _heartbeat_loop(self, task_id, stop_event, state):
        while not stop_event.wait(self.heartbeat_interval):
            heartbeat = tasks.heartbeat(task_id=task_id, worker_id=self.worker_id, lease_seconds=self.lease_seconds)
    ...
    def _start_heartbeat(self, task_id):
        ...
        state = {'lost': False}
        thread = threading.Thread(target=self._heartbeat_loop, args=(task_id, stop_event, state), name=f'sage-heartbeat-{task_id}', daemon=True)
        thread.start()
    ...
    def execute_task(self, task):
        ...
        try:
            ...
            result = orchestrator.execute_goal(goal=description, session_id=session_id, priority=priority, task_id=task_id, worker_id=self.worker_id)
            if heartbeat_state['lost']:
    ...
    def run_once(self):
        ...
        if task.get('status') == 'deferred':
            return {'success': True, 'task': None, **task}

        task_id = task['id']
        try:
            result = self.execute_task(task)
            completed = tasks.complete_claim(task_id=task_id, worker_id=self.worker_id, result=str(result))
            if completed is None:
                return {'success': False, 'task': None, 'error': 'Worker lost task ownership before completion.'}
            return {'success': True, 'task': completed}
        except Exception as error:
            delay = self.retry_base_seconds * (2 ** max(0, task.get('retries', 0)))
            failed = tasks.fail_claim(task_id=task_id, worker_id=self.worker_id, error=str(error), retry_delay_seconds=delay)
            return {'success': False, 'task': failed, 'error': str(error)}
