...
def create_session():
    ...
    try:
        ...
        return {"success": True, "session": {"id": str(session_id)}}
    finally:
        db.close()
...
def execute(request: ExecuteRequest):
    try:
        ...
        if mission_id is None:
            raise RuntimeError("Planner completed but did not return a mission ID.")
...
def get_memory():
    ...
    try:
        ...
        return {"success": True, "memories": _serialize(memories)}
    finally:
        db.close()
...
def add_memory(request: MemoryRequest):
    ...
    try:
        ...
        return {"success": True, "memory": _serialize(memory)}
    finally:
        db.close()
...
def brain_routing(description: str):
    try:
        return {"success": True, "routing": _serialize(router.routing(description))}
    except Exception as exc:
        return {"success": False, "error": str(exc)}
...
@app.get("/tools")
def list_tools():
    return {"success": True, "tools": _serialize(registry.list())}


@app.get("/tools/schemas")
def tool_schemas():
    return {"success": True, "schemas": _serialize(registry.schemas())}


@app.post("/tools/execute")
def execute_tool(tool_name: str, arguments: Optional[str] = None):
    try:
        ...
        result = sage._execute_tool(tool_name, parsed_arguments)
        ...
        return {
            "success": False,
            "tool": tool_name,
            "error": result.get("error", "Tool execution failed.") if isinstance(result, dict) else "Tool execution failed.",
        }
    except Exception as exc:
...
@app.get("/permissions")
def get_permissions():
    return {"success": True, "permissions": _serialize(permissions.get_permissions())}


@app.post("/permissions")
def set_permission(request: PermissionRequest):
    permissions.set_permission(request.permission, request.allowed)
    return {
        "success": True,
...
def audit():
    ...
    try:
        ...
        return {"success": True, "actions": _serialize(actions)}
    finally:
        db.close()
...
def audit_legacy():
    return {"success": True, "audit": _serialize(permissions.audit())}
...
def get_tasks(status: Optional[str] = None):
    ...
    return {"success": True, "tasks": tasks.list(status=status)}
...
def create_mission(request: MissionCreateRequest):
    ...
    return {"success": True, "mission": _serialize(mission)}
...
def plan_mission(request: MissionPlanRequest):
    try:
        ...
        return {"success": True, "plan": _serialize(result)}
    except Exception as exc:
        return {"success": False, "error": str(exc)}
...
def mission_tasks(mission_id: str):
    return {"success": True, "tasks": _serialize(mission_engine.get_tasks(mission_id))}


@app.get("/missions/{mission_id}/ready")
def mission_ready_tasks(mission_id: str):
    return {"success": True, "tasks": _serialize(mission_engine.get_ready_tasks(mission_id))}
...
def start_task(task_id: str):
    return {"success": True, "task": _serialize(mission_engine.start_task(task_id))}


@app.post("/missions/tasks/{task_id}/complete")
def complete_task(task_id: str, request: TaskStatusRequest):
    return {
        "success": True,
        "task": _serialize(mission_engine.complete_task(task_id, result=request.result)),
    }


@app.post("/missions/tasks/{task_id}/fail")
def fail_task(task_id: str, request: TaskStatusRequest):
    return {
        "success": True,
        "task": _serialize(mission_engine.fail_task(task_id, error=request.error or "Task failed.")),
    }
...
def verify_task(task_id: str):
    ...
    return {"success": True, "task": _serialize(result)}
...
def refresh_mission(mission_id: str):
    return {
        "success": True,
        "mission": _serialize(mission_engine.refresh_mission_status(mission_id)),
    }
...
def execute_mission(mission_id: str, request: MissionExecuteRequest):
    try:
...
def execute_next_task(mission_id: str):
    try:
        return _serialize(execution_engine.execute_next(mission_id))
    except Exception as exc:
        return {"success": False, "error": str(exc)}
...
def execute_single_task(task_id: str):
    try:
        return _serialize(execution_engine.execute_task(task_id))
    except Exception as exc:
        return {"success": False, "error": str(exc)}
...
def get_mission_trace(mission_id: str):
    return _serialize(execution_trace.get_mission_trace(mission_id))


@app.get("/missions/{mission_id}/result")
def get_mission_result(mission_id: str):
    return _serialize(execution_trace.get_mission_result(mission_id))


@app.get("/missions/{mission_id}/trace/summary")
def get_trace_summary(mission_id: str):
    trace = execution_trace.get_mission_trace(mission_id)
    if not trace.get("success"):
        return trace
    return {
        "success": True,
        "mission_id": mission_id,
        "summary": _serialize(trace.get("summary")),
    }
...
def list_agents():
    try:
        return {"success": True, "agents": _serialize(agents.list())}
    except Exception as exc:
        return {"success": False, "error": str(exc)}
...
def get_agent(agent_name: str):
    try:
        ...
        return {"success": True, "agent": _serialize(agent)}
